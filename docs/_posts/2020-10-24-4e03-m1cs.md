...
$\lambda_i$: Arrival rate at device *i*  
$\rho_i$: Utilization of device *i*  
$\mu_i$: Processing rate at device *i*  
$M$: Number of clients in the system  
$D$: Average total demand  
$D_{\max}$: Bottleneck demand  
$E[S_i]$: Average processing time at device *i*  
$E[V_i]$: Average expected number of visits to device *i*  
$E[D_i]$: Average demand at node *i*  
$E[N_i]$: Average number of jobs at node *i*  
$E[T_i]$: Expected response time at node *i*  
$E[Z]$: Average user think time  
//////  
$$E[S_i] = \frac{1}{\mu_i}$$  
$$X_i = E[V_i] X$$  
$$E[D_i] = E[S_i] E[V_i]$$  
...
$$E[T] = \sum_{i=1}^K E[V_i] E[T_i] = \frac{M}{X} - E[Z]$$  
$$D = E[D_1] + E[D_2] + ... + E[D_K]$$  
$$D_{\max} = \max_i E[D_i]$$  
$$X \leq \min (\frac{1}{D_{\max}}{\frac{M}{D + E[Z]}})$$  
$$E[T] \geq \max (D, MD_{\max} - E[Z])$$  
...
$$P(X = a) = \frac{e^{-\lambda} \lambda^k}{k!}, k = 0, 1, 2, ...$$  
$$E[X] = V(X) = \lambda$$  
...
- **Blocking probability** is the probability thay tje susyem is full (mean number of jobs lost per unit time), calculated by:  
$$\lambda \pi_N$$
...
$$\pi_n = \frac{1}{n!} (\frac{\lambda}{\mu})^n e^{-\lambda / \mu}$$
    - Poisson distribution with mean $E[N]$
    - $E[T] = 1/\mu$
...
- Some further equations in slides for $E[N_Q], E[T_Q], E[T], E[N]$
- **Erlang-C Formula**  
