...
M/M/*c*/*c* Queue
- Loss system, no waiting. If a job arrives and finds all servers busy, it is lost
- For $n \leq c$:  
$$\pi_n = \frac1{n!} \left(\frac{\lambda}{\mu} \right)^n \pi_0$$  
$$\pi_0 = \frac{1}{1 + \sum_{n=1}^c \frac{1}{n!}(\frac{\lambda}{\mu})^n}$$  
$$\pi_c = \frac{\frac{1}{c!} \left( \frac{\lambda}{\mu} \right)^c}{1 + \sum_{n=1}^c \frac{1}{n!}(\frac{\lambda}{\mu})^n}$$  

Client/Server Model
- Think times of clients exponentially distributed with rate $\lambda$, processing times exponentially distributed with rate $\mu$
    - No need to keep track of individual jobs, so it doesn't matter what scheduling policy at CPU is
- Equations:  
$$\pi_n = \left( \Pi_{i = 1}^n \frac{\lambda(M - i + 1)}{\mu} \right) \pi_0$$  
$$\pi_n = \left( \left( \frac{\lambda}{\mu} \right)^n \frac{M!}{(M - n)!} \right) \pi_0$$  
...
- **Theorem**: Given an M/M/*k* with arrival rate $\lambda$ and server speed $\mu$ and $R = \lambda/\mu$, where *R* is large, let $k_\alpha^{star}$ denote the least number of servers needed to ensure that $P_Q^{M/M/k} \lt \alpha$. Then:  
$$k_\alpha^* \approx R + c \sqrt{R}$$  
where *c* solves:  
$$\frac{c \Phi}{\phi(c)} = \frac{1 - \alpha}\alpha$$  
where $c \Phi$ is the cdf of a Normal(0,1) and $\phi(c)$ is its density (pdf)
...
where $n_i$ is the number of jobs at $i$ for the given job distribution state
- We need to calculate *C* though, which can be done with the following:  
$$1/C = \sum  \Pi_{i=1}^N \left( \frac{\lambda_i}{\mu_i} \right)^{n_i}$$
...
so our samples would be given by:  
$$x = F^{-1}(u)$$
    - If $F(x)$ is an exponential distribution with rate $\lambda$, we would get:  
    $$F^{-1}(u) = -ln(u)/\lambda$$  
...
- **NOTE:** To reduce the width of the confidence interval by a factor of $n$, we need to increase the number of samples by a factor of $2n$
...
$$R = 1 - (1 - R_1)(1 - R_2) ...$$
- **Triple Modular Redundancy**: System consists with three components, each having the same reliability, where at least two must work:  
...
- $E[S^2]$ can be calculated using the following:  
$$E[S^2] = \sigma_S^2 + (E[S])^2$$
