...
- **Classification of RTS**
    ...
    - A **firm RTS** is one where nussubg a few deadlines will not lead to total failure, but missing more than a few may lead to complete and catastrophic system failure
...
- **Kernel and Kernel Space**
    ...
    - User space is the space in memory where user processes run, and kernel space is the space where kernel processes run
        - User space only alloqs limited instructions and prohibits direct access to the hardware/memory
...
- Two approaches to implement RTOS based on linux:
    ...
    2. Make necessary changes in the Linux Kernel to make it suitable for real time applications (preempt RT)
        - Preempt RT is use on the myRIO devices, so the kernel threads are preemptable - a **kernel module** is a piece of code that can be loaded and unloaded into the kernel upon demand
...
- Linux Priority Levels and Nice Values
    - Linux has static priority ranged from 0 to 139, where 0 to 99 are reserved for real time users, and 100 to 139 for users
